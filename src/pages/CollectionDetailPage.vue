...
async function loadCollectionDetail() {
  ...
  try {
    ...
    if (res.data.code === 200) {
      ...
      toast.error(res.data.message || '获取合集详情失败')
    }
  } catch {
...
function getDisplayStars(item: CollectionRatingItem): number {
...
// 星星评分点击（乐观更新）
async function handleStarClick(item: CollectionRatingItem, star: number, event: Event) {
  ...
  <div class="page-container">
    ...
    <main class="page-content">
      ...
      <template v-if="isLoading">
        <!-- 合集信息骨架 -->
        <div class="collection-header-skeleton">
      ...
      <!-- 空状态 -->
      <div v-else-if="!collection" class="empty-container">
      ...
      <template v-else>
        ...
        <div class="collection-header">
          ...
          <div class="collection-info">
            ...
            <span class="collection-count">{{ collection.itemCount }} 个评分项目</span>
          </div>
        </div>

        <!-- 空列表状态 -->
        <div v-if="collection.items.length === 0" class="empty-container">
          <div class="empty-icon">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        ...
        <template v-else>
          ...
          <div class="filter-section">
            <span class="total-count">全部评分 / {{ collection.items.length }}</span>
...
:root.dark .collection-cover-placeholder {
  background: linear-gradient(135deg, #312e81 0%, #3730a3 100%);
}

.collection-cover-placeholder svg {
  width: 40px;
  height: 40px;
}
...
.collection-header-skeleton .skeleton-cover {
  width: 100px;
  height: 100px;
  background: var(--color-border);
  border-radius: var(--radius-md);
...
.collection-header-skeleton .skeleton-info {
  flex: 1;
  display: flex;
  flex-direction: column;
  justify-content: center;
...
.collection-header-skeleton .skeleton-title {
  width: 60%;
  height: 24px;
  background: var(--color-border);
...
.collection-header-skeleton .skeleton-desc {
  width: 80%;
  height: 16px;
  background: var(--color-border);
  border-radius: var(--radius-sm);
