...
// 小分区ID
const minorSectionId = computed(() => Number(route.params.minorSectionId))

// 加载状态
const isLoading = ref(true)

// 小分区信息
const minorSection = ref<AdminMinorSection | null>(null)
...
// 加载小分区信息
async function loadMinorSection() {
  try {
    const res = await getAdminMinorSectionDetail(minorSectionId.value)
    if (res.data.code === 200) {
      minorSection.value = res.data.data
    } else {
      toast.error('获取小分区信息失败')
...
async function loadRatingItems() {
  try {
    const params: {
      ...
      page: page.value,
      size: pageSize.value,
      minorSectionId: minorSectionId.value,
    }
...
async function loadAllData() {
  ...
  try {
    await Promise.all([loadMinorSection(), loadRatingItems()])
  } finally {
...
function openCreateModal() {
  ...
  form.value = { minorSectionId: minorSectionId.value, name: '', description: '' }
...
function openEditModal(item: AdminRatingItem, event: Event) {
  ...
  form.value = {
    minorSectionId: item.minorSectionId,
    name: item.name,
...
// 返回
function goBack() {
  if (minorSection.value) {
    router.push(`/admin/rating/major-sections/${minorSection.value.majorSectionId}`)
  } else {
    router.push('/admin/rating')
  }
}
...
<template>
  <div class="page-container">
    <PageHeader :back-to="minorSection ? `/admin/rating/major-sections/${minorSection.majorSectionId}` : '/admin/rating'" />
        ...
        <div class="page-header-section">
          <div class="header-main">
            <div class="header-text">
              <div class="header-breadcrumb" v-if="minorSection">
                <span class="breadcrumb-item">{{ minorSection.schoolName }}</span>
                <span class="breadcrumb-sep">/</span>
                <span class="breadcrumb-item">{{ minorSection.majorSectionName }}</span>
                <span class="breadcrumb-sep">/</span>
                <span class="breadcrumb-current">{{ minorSection.name }}</span>
              </div>
              <h1 class="page-title">{{ minorSection?.name || '加载中...' }}</h1>
