...
import {
  getAdminMajorSectionDetail,
  getAdminMinorSections,
  createMinorSection,
  updateMinorSection,
  deleteMinorSection,
  updateMinorSectionStatus,
  uploadMinorSectionImage,
  deleteMinorSectionImage,
} from '@/api/rating'
import type {
  AdminMajorSection,
  AdminMinorSection,
...
// 大分区ID
const majorSectionId = computed(() => Number(route.params.majorSectionId))

// 加载状态
const isLoading = ref(true)

// 大分区信息
const majorSection = ref<AdminMajorSection | null>(null)

// 小分区列表
const minorSections = ref<AdminMinorSection[]>([])
const total = ref(0)
const page = ref(1)
...
const editingItem = ref<AdminMinorSection | null>(null)
const form = ref<CreateMinorSectionRequest & { description: string }>({
  majorSectionId: 0,
  name: '',
  description: '',
})
...
const deleteTarget = ref<AdminMinorSection | null>(null)
const isDeleting = ref(false)

// 加载大分区信息
async function loadMajorSection() {
  try {
    const res = await getAdminMajorSectionDetail(majorSectionId.value)
    if (res.data.code === 200) {
      majorSection.value = res.data.data
    } else {
      toast.error('获取大分区信息失败')
...
async function loadMinorSections() {
  try {
    const params: { page?: number; size?: number; majorSectionId?: number; status?: number } = {
      page: page.value,
      size: pageSize.value,
      majorSectionId: majorSectionId.value,
    }
    if (statusFilter.value !== 'all') {
      params.status = statusFilter.value
    }
    const res = await getAdminMinorSections(params)
    if (res.data.code === 200) {
      minorSections.value = res.data.data.items
      total.value = res.data.data.total
    } else {
      toast.error(res.data.message || '获取小分区列表失败')
    }
  } catch (error) {
    toast.error('获取小分区列表失败')
  }
}
...
async function loadAllData() {
  ...
  try {
    await Promise.all([loadMajorSection(), loadMinorSections()])
  } finally {
...
function openCreateModal() {
  ...
  form.value = { majorSectionId: majorSectionId.value, name: '', description: '' }
...
// 打开编辑模态框
function openEditModal(item: AdminMinorSection, event: Event) {
...
async function save() {
  ...
  try {
    if (modalMode.value === 'create') {
      const res = await createMinorSection(form.value)
      if (res.data.code === 200) {
        if (imageFile.value) {
          await uploadMinorSectionImage(res.data.data.id, imageFile.value)
        }
        toast.success('创建成功')
        closeModal()
        await loadMinorSections()
      } else {
        toast.error(res.data.message || '创建失败')
      }
    } else if (editingItem.value) {
      const updateData: UpdateMinorSectionRequest = {
        name: form.value.name,
        description: form.value.description,
      }
      const res = await updateMinorSection(editingItem.value.id, updateData)
      if (res.data.code === 200) {
        if (imageFile.value) {
          await uploadMinorSectionImage(editingItem.value.id, imageFile.value)
        } else if (!imagePreview.value && editingItem.value.imageUrl) {
          await deleteMinorSectionImage(editingItem.value.id)
        }
        toast.success('更新成功')
        closeModal()
        await loadMinorSections()
...
// 切换状态
async function toggleStatus(item: AdminMinorSection, event: Event) {
  event.stopPropagation()
  const newStatus = item.status === 1 ? 0 : 1
  try {
    const res = await updateMinorSectionStatus(item.id, { status: newStatus as RatingStatus })
    if (res.data.code === 200) {
...
// 打开删除确认
function openDeleteConfirm(item: AdminMinorSection, event: Event) {
  event.stopPropagation()
...
async function confirmDelete() {
  ...
  try {
    const res = await deleteMinorSection(deleteTarget.value.id)
    if (res.data.code === 200) {
      toast.success('已删除')
...
// 进入评分项目列表
function goToRatingItems(item: AdminMinorSection) {
  router.push(`/admin/rating/minor-sections/${item.id}`)
}

// 返回
function goBack() {
  if (majorSection.value) {
    router.push(`/admin/rating/schools/${majorSection.value.schoolId}`)
  } else {
    router.push('/admin/rating')
  }
}
...
<template>
  <div class="page-container">
    <PageHeader :back-to="majorSection ? `/admin/rating/schools/${majorSection.schoolId}` : '/admin/rating'" />
    ...
    <main class="page-content">
      <div class="content-container">
        ...
        <div class="page-header-section">
          <div class="header-main">
            <div class="header-text">
              <div class="header-breadcrumb" v-if="majorSection">
                <span class="breadcrumb-school">{{ majorSection.schoolName }}</span>
                <span class="breadcrumb-sep">/</span>
                <span class="breadcrumb-current">{{ majorSection.name }}</span>
              </div>
              <h1 class="page-title">{{ majorSection?.name || '加载中...' }}</h1>
              <p class="page-subtitle">管理小分区</p>
            </div>
        ...
        <template v-else>
          ...
          >
            <!-- 空状态 -->
            <div v-if="minorSections.length === 0" class="empty-container">
              ...
              <h2>{{ statusFilter === 'all' ? '暂无小分区' : '暂无相关小分区' }}</h2>
              <p>{{ statusFilter === 'all' ? '还没有添加任何小分区' : '没有找到该状态的小分区' }}</p>
            ...
            <div v-else class="items-list">
              <div
                v-for="item in minorSections"
                :key="item.id"
                class="item-card"
                @click="goToRatingItems(item)"
              >
                <!-- 封面图 -->
                ...
                <div class="item-info">
                  ...
                  <div class="item-meta">
                    <span class="meta-item">{{ item.ratingItemCount }} 个评分项目</span>
      ...
      <div v-if="showDeleteConfirm" class="modal-overlay" @click.self="cancelDelete">
        <Transition name="modal-scale" appear>
          <div v-if="showDeleteConfirm" class="modal-content" @click.stop>
            ...
            <p class="modal-desc">
              确定要删除小分区"{{ deleteTarget?.name }}"吗？此操作不可撤销。
            </p>
            <p class="modal-warning">注意：如果该分区下有评分项目，将无法删除。</p>
            <div class="modal-actions">
